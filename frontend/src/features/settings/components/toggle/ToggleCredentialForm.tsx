...
import {
  ...
  type ToggleCredentialFormData,
} from "../../schemas/toggleCredential";
import type { components } from "@/types/api";

type ToggleCredential = components["schemas"]["ToggleCredentialResponseDto"];

interface ToggleCredentialFormProps {
  initialData?: ToggleCredential | null;
  onSave: (data: ToggleCredentialFormData) => Promise<void>;
  onCancel: () => void;
}

export function ToggleCredentialForm({
  initialData,
  onSave,
  onCancel,
}: ToggleCredentialFormProps) {
  ...
  const form = useForm({
    ...
    } as ToggleCredentialFormData,

    onSubmit: async ({ value }) => {
      setIsSaving(true);
      try {
        const result = toggleCredentialSchema.safeParse(value);
        if (!result.success) {
            ...
            {(field) => (
              <View className="mb-4">
                ...
                <Text className="text-gray-500 text-xs mt-1">
                  Toggle Track → Profile Settings → API Tokenで取得
                </Text>
              </View>
            )}
          </form.Field>
